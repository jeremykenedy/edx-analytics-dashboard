...
def _filter_children(blocks, key, **kwargs):
    ...
    for child in block[u'children']:
        children += _filter_children(blocks, child, **kwargs)
...
def course_structure_to_assignments(structure, graded=None, assignment_type=None):
    ...
    # Break down the course structure into assignments and nested problems, returning only the data we absolutely need.
    assignments = []
    ...
    filtered = _filter_children(blocks, root[u'id'], **kwargs)

    for assignment in filtered:
        filtered_children = _filter_children(blocks, assignment[u'id'], graded=graded, block_type=u'problem')
        problems = []
